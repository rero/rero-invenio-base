...
def test_cli_es_index_alias(
    ...
    res = runner.invoke(
        switch_index, [new_index_name1, new_index_name2], obj=script_info
    )
    assert res.exit_code == 0
    ...
    res = runner.invoke(
        delete_alias, [new_index_name2, "records", "--yes-i-know"], obj=script_info
    )
    assert res.exit_code == 0
...
def test_cli_es_snapshots(script_info, app, es_runner, new_index_name1):
    ...
    assert res.exit_code == 0
    res = runner.invoke(
        delete_snapshot, ["snap", "test", "--yes-i-know"], obj=script_info
    )
    assert res.exit_code == 0
